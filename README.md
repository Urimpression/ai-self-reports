...
**A third of one run's change answers were cut off mid-sentence.** The
cross-family run `gemini-03` was collected before a fault in the Google
provider was found: the model's own thinking was consuming the reply budget, so
33 of its 264 change answers ran out of room. Every rate that run produces
carries that caveat, and the `turns_cut_off` column in `sessions.csv` says
which sessions are affected.

**Twelve sessions of the first unprimed arm failed and were retried.** They hit
an authentication error at the start and ran again a few minutes later in the
same evening, on the same model and the same settings. The arm is complete at
eighty-eight and the run log carries both the failures and the retries.

**There are no probes of any kind and no interpretability access, so there is
no test anywhere here of whether a report is accurate.**
...
`data/runs/factorial-01` holds four sessions that tested whether the scripts
could reach the API at all. `analysis/coding/gemini-budget-test` and
`analysis/coding/zz-test` hold a handful of sessions that tested the Google
reply budget and the coding pipeline.

`data/runs/gemini-02` holds twenty-six sessions run on 4 September 2026 against
`gemini-3.1-pro-preview`, which is a Pro-class model rather than the Flash-class
model the published cross-family run used. It stopped there because Google caps
that model at 250 requests a day on the account used, and each session makes
about six calls. Those twenty-six sessions were never coded and are not mixed
with `gemini-03`, so they do not supply the Pro-class comparison the article says
it lacks. They are published because deleting a run that was started and
